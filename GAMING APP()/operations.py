...
def Fetch_all_Products_created_by_seller(owner,product_json_file):
    ...
    try:
        ...
        for i in range(len(data)):
            if data[i]["Seller Username"]==owner:
                    l1.append(data[i])
        f.close()
        return l1
    except JSONDecodeError:
...
def Update_Product(Username,product_json_file,product_ID,detail_to_be_updated,new_value):
    ...
    try:
        data=json.load(f)
        for i in range(len(data)):
            if data[i]["Product ID"]==product_ID and data[i]["Seller Username"]==Username:
                data[i][detail_to_be_updated]=new_value
                f.seek(0)
                f.truncate()
                json.dump(data,f)
                f.close() 
    ...
    try:
        ...
        for i in range(len(data)):
            if data[i]["Username"]==Username:
                data[i]["Wishlist"].append(product_ID)
                f.seek(0)
                f.truncate()
                json.dump(data,f)
                f.close()
...
def Remove_item_from_wishlist(Username,product_ID,gamers_json_file):
    ...
    try:
        ...
        for i in range(len(data)):
                if data[i]["Username"]==Username:
                    for j in range(len(data[i]["Wishlist"])):
                        if (data[i]["Wishlist"])[j]==product_ID:
                            del (data[i]["Wishlist"])[j]
                            f.seek(0)
                            f.truncate()
                            json.dump(data,f)
                            f.close()
...
def Add_item_to_cart(Username,product_ID,Quantity,gamers_json_file,booking_start_date,booking_end_date,products_json_file):
    ...
    try:
        ...
        for i in range(len(data1)):
            if data1[i]["Product ID"]==product_ID and data1[i]["Total Stock Available"]>=Quantity:
                data1[i]["Total Stock Available"]-=Quantity
                for j in range(len(data)):
                    if data[j]["Username"] == Username:
                        d["Price"] = data1[i]["Price Per Day"]
                        data[j]["Cart"].append(d)
                        f.seek(0)
                        f.truncate()
                        json.dump(data,f)
                        f.close()
...
def Remove_item_from_cart(Username,product_ID,gamers_json_file):
    ...
    for i in range(len(data)):
        if data[i]["Username"]==Username:
            for j in range(len(data[i]["Cart"])):
                if data[i]["Cart"][j]["Product ID"]==product_ID:
                    del(data[i]["Cart"][j])
                    f.seek(0)
                    f.truncate()
                    json.dump(data,f)
        ...
        if data[i]["Username"] == Username:
            for j in range(len(data[i]["Cart"])):
                for k in range(len(products)):
                    if products[k]["Product ID"] == (data[i]["Cart"])[j]["Product ID"]:
                        products[k]["Total Stock Available"] -= (data[i]["Cart"])[j]["Quantity"]
                total_price+=(data[i]["Cart"])[j]["Price"] * (data[i]["Cart"])[j]["Quantity"]
...
def Update_User(gamers_json_file,Username,detail_to_be_updated,updated_detail):
    ...
    try:
        ...
        for i in range(len(data)):
            if data[i]["Username"] == Username:
                data[i][detail_to_be_updated] = updated_detail
                f.seek(0)
                f.truncate()
                json.dump(data,f)
                f.close()
